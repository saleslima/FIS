...
function setupBookingFormHandlers(dateKey, periodIndex, day) {
    ...
    const validateForm = () => {
        ...
        if (!isMilitar) {
            ...
            confirmBtn.disabled = !(nameInput.value.trim().length>0 && cpf.length===11 && ph.length===11);
        } else {
            const ph = (phoneMInput.value||'').trim(); const re = (reInput.value||'').trim();
            confirmBtn.disabled = !(warNameInput.value.trim().length>0 && gradInput.value && re.length>=1 && ph.length===11);
        }
    };
    ...
    confirmBtn.addEventListener('click', () => {
        ...
        if (!isMilitar) {
            ...
            bookingData = { type:'civil', name:nameInput.value.trim(), cpf, phone };
        } else {
            ...
            bookingData = { type:'militar', warName:warNameInput.value.trim(), graduation:gradInput.value, re, phone };
        }
